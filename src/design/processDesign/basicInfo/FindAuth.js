...
const FindAuth = props =>{
    ...
    const {pipelineStore,taskStore,authStore,serverStore,hostStore}=props
    ...
    useEffect(()=>{
        // 初始化选择框list
        dataItem.taskType && initList(dataItem.taskType)
    },[dataItem.taskType,authFresh,serverFresh,hostFresh])
    ...
    const initList = taskType =>{
        switch (taskType) {
            ...
            case 41:
            case 51:
                findAllAuthServerList(dataItem.taskType).then(res=>{getList(res)})
                break
            case 31:
            case 32:
            case 52:
...
export default inject("pipelineStore","taskStore","authStore"
